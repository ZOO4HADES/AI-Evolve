...
class ChartV3 {
    ...
    async renderMonthWithPixelAnimation(monthData) {
        ...
        if (this._pendingElements && this._pendingElements.size > 0) {
            console.log(`[ChartV3] 先插入 ${this._pendingElements.size} 个新元素到DOM`);
            ...
            this._pendingElements.forEach((element, modelName) => {
                ...
                console.log(`[ChartV3] 插入新元素 ${modelName} (排名${currentRank})`);
            });
...
// ⚠️ 关键修复：先获取新位置（基于当前DOM顺序），然后再重新排序DOM
        ...
        animations.forEach(anim => {
            if (anim.type === 'move') {
                ...
                console.log(`[ChartV3] 初始transform应用到 ${anim.element.dataset.modelName}: translate3d(${anim.deltaX.toFixed(1)}px, ${anim.deltaY.toFixed(1)}px, 0)`);
        ...
        if (isFirstRender) {
            console.log('[ChartV3] ========== 首次渲染模式 ==========');
            const allElements = Array.from(this.currentElements.values());
            console.log(`[ChartV3] 找到 ${allElements.length} 个元素`);
            ...
            allElements.forEach((element, index) => {
                ...
                if (bar) {
                    ...
                    if (!element._targetBarWidth) {
                        ...
                        console.log(`[ChartV3] ${index}. ${modelName}: 目标宽度=${targetWidth}（首次读取）`);
                    } else {
                        console.log(`[ChartV3] ${index}. ${modelName}: 目标宽度=${element._targetBarWidth}（已存在，保留）`);
                    }
                    bar.style.width = '0%';
                }
                ...
                console.log(`[ChartV3] ${modelName}: 设置_isFirstRender=true`);
            });
            console.log('[ChartV3] 首次渲染：所有柱状图设置为0，准备播放增长动画');
        }
    ...
    calculateAnimations(oldPositions, newPositions) {
        ...
        console.log('[ChartV3] ========== 计算动画指令 ==========');
        console.log('[ChartV3] 旧位置数量:', oldPositions.size);
        console.log('[ChartV3] 新位置数量:', newPositions.size);
...
if (Math.abs(deltaY) > 0.5 || Math.abs(deltaX) > 0.5) {
                ...
                // 新元素
                console.log('[ChartV3] 新元素:', modelName);
                animations.push({
                    type: 'enter',
                    element: element
                });
        ...
        oldPositions.forEach((oldPos, modelName) => {
            if (!newPositions.has(modelName)) {
                console.log('[ChartV3] 离开元素:', modelName);
                animations.push({
                    type: 'leave',
        ...
        console.log('[ChartV3] 总动画数量:', animations.length);
        return animations;
    }
    ...
    updateDOM(monthData, changes) {
        ...
        if (this.currentElements.size === 0) {
            ...
            console.log(`[ChartV3] 锁定容器高度: ${currentHeight}px`);
        }
        ...
        console.log(`[ChartV3] DOM更新完成，${entering.length}个新元素待插入，排序将在动画后应用`);
    }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { ChartV3, COMPANY_COLORS };
}
